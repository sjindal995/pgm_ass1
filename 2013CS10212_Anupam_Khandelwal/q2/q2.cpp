...
double getScore(char* word)
{
	...
	if(mode > 2)
	{
		// int counts[N_IMGS][CHARS] = {0};
		// int k = 0;
		// for(int i=0;i<n;i++)
		// {
		// 	k += ++counts[images[i]][word[i]];
		// }
		// score *= pow(5,k);
		for(int i = 0 ; i < n ; i++)
		{
...
vector<string> getBestWords(char* inputImageFile , char* outFile , int mod)
{
...
int main(int argc , char** argv)
{
	...
	if(argc > 2)
	{
		...
		char* outFile = argv[2];
		getBestWords(imageFile,outFile,3);
	}
	int stop_s=clock();
